...
SELECT *
FROM (
  SELECT c.codi, c.tipus, c.data, c.duració, c.hora, c.codi_sala, c.codi_gimnas, c.tutor
  FROM classes c
  LEFT JOIN participacions p ON c.codi = p.classe AND p.client = '35194442t'
  WHERE p.classe IS NULL
) AS foo
WHERE (c.data > current_date) OR (data = current_date AND hora > current_time)
